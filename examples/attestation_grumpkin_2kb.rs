...
fn main() -> Result<()> {
    // read attestion data
    let attestation_data = fs::read_to_string("data/attestation_data_grumpkin_2kb.json").unwrap();
    // read attestation config
    let attestation_config = fs::read_to_string("data/config_yl_2kb.json")?;
